-file("src/game_client_pb.erl", 1).

-module(game_client_pb).

-export([encode_game_client/1, decode_game_client/1,
	 delimited_decode_game_client/1]).
...
encode_game_client(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_game_client(Record)
    when is_record(Record, game_client) ->
    encode(game_client, Record).

encode(game_client, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(game_client, Record) ->
    [iolist(game_client, Record)
...
iolist(game_client, Record) ->
    [pack(1, required,
	  with_default(Record#game_client.uid, none), int32, []),
     pack(2, optional,
	  with_default(Record#game_client.name, none), string,
	  []),
     pack(3, repeated,
	  with_default(Record#game_client.leave, none), int32,
	  []),
     pack(4, repeated,
	  with_default(Record#game_client.equ_name, none), string,
	  []),
     pack(5, optional,
	  with_default(Record#game_client.equ_leave, none), int32,
	  []),
     pack(6, optional,
	  with_default(Record#game_client.tools_name, none),
	  string, []),
     pack(7, optional,
	  with_default(Record#game_client.tool_num, none), int32,
	  [])].
...
decode_game_client(Bytes) when is_binary(Bytes) ->
    decode(game_client, Bytes).

delimited_decode_game_client(Bytes) ->
    delimited_decode(game_client, Bytes).
...
decode(game_client, Bytes) when is_binary(Bytes) ->
...
to_record(game_client, DecodedTuples) ->
